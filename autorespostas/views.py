...
def _check_response_limit(user):
    ...
    if isinstance(limit_or_none, int):
        ...
        msg = f"Seu plano permite no máximo {limit_or_none} respostas automáticas."
        if display:
            msg += f" Para assinar um plano superior, chame o administrador: {display}"
        return False, msg
...
def boas_vindas_view(request):
    ...
    if plan and not plan.permite_boas_vindas:
        ...
        messages.warning(
            request,
            f"Seu plano não inclui mensagem de boas-vindas. "
            f"Para assinar, chame o administrador: {display}",
        )
...
def horario_view(request):
    ...
    if plan and not plan.permite_horario:
        ...
        messages.warning(
            request,
            f"Horário de atendimento disponível a partir do plano Básico. "
            f"Para assinar, chame o administrador: {display}",
        )
        return redirect("subscriptions:plans")
